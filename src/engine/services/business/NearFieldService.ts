/**
 * NearFieldService - 近场交互业务服务（静态方法）
 * 
 * Phase 6.3: 从 NearFieldServiceImpl 完全迁移到 business 层
 * - 完全无状态（静态方法）
 * - DataAccess 在方法内部创建
 * - 负责处理近场交互的所有逻辑
 * 
...
export class NearFieldService {
   ...
   * 推进场景状态（统一入口）
   * 
   * 🔥 从 NearFieldServiceImpl 迁移，改为静态方法
   * 
   ...
   */
  static async advance(request: AdvanceRequest): Promise<AdvanceResponse> {
    ...
    switch (player_action.type) {
      ...
      case "INTERACT":
        const sceneData = await this.getSceneData(story_id, current_scene_id, sceneDataAccess);
   ...
   * 
   * 🔥 从 NearFieldServiceImpl 迁移
   * 
   * @param storyId 故事ID
   * @param sceneId 场景ID
   * @param sceneDataAccess 场景数据访问接口
   * @returns 场景数据
   * @throws Error 如果场景不存在
   */
  private static async getSceneData(
    storyId: string, 
    sceneId: string,
    sceneDataAccess: ISceneDataAccess
  ): Promise<any> {
    const sceneData = await sceneDataAccess.getAllSceneMocks(storyId, sceneId);
   ...
   * @returns advance响应
   */
  private static async handleLoadScene(
    storyId: string,
    sceneId: string,
   ...
   * @returns advance响应
   */
  private static async handleInteract(
    storyId: string,
    sceneId: string,
    sceneData: any, 
   ...
   * @returns advance响应
   */
  private static async handlePass(
    storyId: string,
    sceneId: string,
    narrativeIndex: number
  ): Promise<AdvanceResponse> {
   ...
   * @returns advance响应
   */
  private static async handleRequestNarrative(
    storyId: string,
    sceneId: string,
    narrativeIndex: number,
    storyDataAccess: IStoryDataAccess
   ...
   * @returns 当前轮次（从1开始）
   */
  private static calculateTurn(history: NearFieldEvent[]): number {
   ...
   * @returns 填充后的响应
   */
  private static fillPlayerIntentText(
